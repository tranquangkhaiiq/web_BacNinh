...
namespace webvl2024_BacNinh.Controllers
{
    public class AccountController : Controller
    {
            ...
            if (captcha != null)
            {
                ...
                if (captcha == getcaptcha)
                {
                    ...
                    else if (user.Count == 0)
                    {
                        ...
                        string[] user_register = new string[5];
        ...
        [HttpPost]
        [ValidateInput(false)]
        public ActionResult _JobAccount(DoanhNghiep dn)//DoanhNghiep đăng ký
        {
            ViewBag.LoaiHinhDoanhNghiep_ID = new SelectList(dbc.DM_LoaiHinhDoanhNghiep.ToList(), "LoaiHinhDoanhNghiep_ID", "TenLoaiHinhDoanhNghiep");
            ViewBag.Tinh_ID = new SelectList(dbc.DM_DiaChi.Where(nh => nh.ParentId == 0 && nh.KichHoat == true).OrderByDescending(nh => nh.MoTa), "Id", "TenDiaChi", TT_Tinh);
            ViewBag.ChucVu = new SelectList(dbc.DM_ChucDanh.ToList(), "ChucDanh_ID", "TenChucDanh");
            ...
            if (Session["User_re"] != null)
            {
                try
                {
                    var user_register = new string[5];
                    ...
                    var InserUserDN = new DAO.NhatKy_Admin_DAO().InsertUserWeb(
                        user_register[0].ToString(), user_register[3].ToString(), user_register[4].ToString()
                        , model.DN_ID, PasswordSalt, CirpherPass, 4);
                }
                catch (Exception ex)
                {
        ...
        public ActionResult _JobCategory()
        {
            if (Session["UsrID"] != null)
            {
                ...
                ViewBag.TotalDaLuu = dbc.Database.SqlQuery<PHU_HSTV_Luu>("select t1.Id,t1.UserId,t1.NTV_Id,t1.HSTV_Id,t1.TenHoSo,t1.NgayTao " +
                    "from [VLBN].[dbo].[PHU_HSTV_Luu] t1 " +
                    "inner join " +
                    "[VLBN].[dbo].[KhachHang_TimViecLam] t2 on t1.HSTV_Id = t2.TimViec_ID" +
        ...
        public ActionResult _JobInfo()
        {
            if (Session["UsrID"] != null)
            {
                ...
                if (DN_ID != null)
                {
                    ViewBag.DoanhNghiep = dbc.Database.SqlQuery<DoanhNghiep>("select * from [VLBN].[dbo].[DoanhNghiep] where DN_ID=" + DN_ID.UserChild_id);
                    return PartialView();
                }
                else
                {
        ...
        public ActionResult _JobManagement()
        {
            if (Session["UsrID"] != null)
            {
                ...
                for (int i = 0; i < listTDbyDN.Count; i++)
                {
                    ...
                    //var update = dbc.Database.ExecuteSqlCommand("update VLBN.dbo.[DoanhNghiep_UngTuyen] set DN_Daxem=1 where TuyenDung_ID=@Id",
        ...
        public ActionResult _JobHSPhuHop()
        {
            if (Session["UsrID"] != null)
            {
                int uID = int.Parse(Session["UsrID"].ToString());
        ...
        public ActionResult DeleteJobDoc(int Id)
        {
            ...
            if (model != null)
            {
                try
                {
                    var XoaNLD = dbc.Database.ExecuteSqlCommand("DELETE  FROM [VLBN].[dbo].[DoanhNghiep_TuyenDung_NghiepVu] where TuyenDung_ID=" + model.TuyenDung_ID);
                    var XoaNLD2 = dbc.Database.ExecuteSqlCommand("DELETE  FROM [VLBN].[dbo].[DoanhNghiep_TuyenDung_NgoaiNgu] where TuyenDung_ID=" + model.TuyenDung_ID);
                    var XoaNLD3 = dbc.Database.ExecuteSqlCommand("DELETE  FROM [VLBN].[dbo].[DoanhNghiep_TuyenDung_TinHoc] where TuyenDung_ID=" + model.TuyenDung_ID);
        ...
        [HttpPost]
        [ValidateInput(false)]
        public ActionResult _EditJobAccount(DoanhNghiep modleDN, DateTime? NgayThanhLap1,
        ...
        public ActionResult DeleteCandidateSave(int Id)
        {
            ...
            try
            {
                ...
                Session["ThongBao_DN_TD"] = "Xóa thành công 1 hồ sơ ứng viên do bạn đã lưu.";
                return View("_MainJob");
            }
            catch
            {
        ...
        public ActionResult UpdateDocFile(DoanhNghiep_TuyenDung model, DateTime? NgayNhanHoSo1, DateTime NgayHetHan1, int NghiepVu1, int NghiepVu11, int NghiepVu2, int NghiepVu22, int NghiepVu3, string Tieude_kh)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    var update = dbc.Database.ExecuteSqlCommand("update VLBN.dbo.[DoanhNghiep_TuyenDung] set [TieuDeTuyenDung]=@TenHoSo," +
        ...
        public ActionResult thaydoitrangthai(int tuyendungid)
        {
            ...
            var update = dbc.Database.ExecuteSqlCommand("update VLBN.dbo.[DoanhNghiep_TuyenDung] set HienThiWeb=@HienThiWeb where TuyenDung_ID=@Id",
        ...
        //                    var update = dbc.Database.ExecuteSqlCommand("update VLBN.dbo.[DoanhNghiep_TuyenDung] set KichHoat=0 where TuyenDung_ID=@Id",
        //                new SqlParameter("@Id", tuyendungid));
                        ...
                        if (kq > 0)
                        {
                            ...
                            {//update Pay_toncuoi
                                ...
                                if (kup > 0)
                                {
                                    ...
                                    if (tuyendung != null)
                                    {
                                        var update = dbc.Database.ExecuteSqlCommand("update VLBN.dbo.[DoanhNghiep_TuyenDung] set TinhTrangHoSo=2 where TuyenDung_ID=@Id",
                                            new SqlParameter("@Id", tuyendungid));
                                        if (update > 0)
                                        {
            ...
            if (Session["User_re"] != null)
            {
                try
                {
                    ...
                    var insertUserkhachhang = new DAO.NhatKy_Admin_DAO().InsertUserWeb(
                        user_register[0].ToString(), user_register[3].ToString(), user_register[4].ToString()
                        , model.KH_ID, PasswordSalt, CirpherPass, 3);
        ...
        public ActionResult _CandidateCategory()
        {
            if (Session["UsrID"] != null)
            {
                ...
                ViewBag.TotalHSDaLuu = dbc.Database.SqlQuery<PHU_HSTD_Luu>("select * from [VLBN].[dbo].[PHU_HSTD_Luu] where UserId =" + uID
        ...
        public ActionResult _CandidateDoc()
        {
            if (Session["UsrID"] != null)
            {
                ...
                if (KH_ID != null)
                {
                    var NTVId = dbc.Database.SqlQuery<int>("select KH_ID from [VLBN].[dbo].[KhachHang] where KH_ID=" + KH_ID.UserChild_id);

                    ViewBag.DSHSbyNTV = dbc.Database.SqlQuery<KhachHang_TimViecLam>("select * from [VLBN].[dbo].[KhachHang_TimViecLam] where KH_ID=" + NTVId.First());
                    ViewBag.DSknCV = dbc.Database.SqlQuery<KhachHang_KinhNghiem_LamViec_2022>("select * from [VLBN].[dbo].[KhachHang_KinhNghiem_LamViec_2022] where KH_ID=" + NTVId.First());
        ...
        public ActionResult _CandidateInfo()
        {
            if (Session["UsrID"] != null)
            {
                ...
                ViewBag.NTV = dbc.Database.SqlQuery<KhachHang>("select * from [VLBN].[dbo].[KhachHang] where KH_ID=" + user.UserChild_id);
                return PartialView();
            }
            return Json("No", JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult _EditCandidateAccount(KhachHang kh_td, string HoTen_kh, string DienThoai_kh, string Email_kh
            , int TrinhDo_ID, int Nganh_ID, int Nghe_ID, int NghiepVu1, int NghiepVu11, int NghiepVu2
        ...
        public ActionResult thaydoitrangthai_kh(int tuyendungid)
        {
            ...
            var update = dbc.Database.ExecuteSqlCommand("update VLBN.dbo.[KhachHang_TimViecLam] set HienThiTrenWeb=@HienThiTrenWeb where TimViec_ID=@Id",
        ...
        public ActionResult _CandidateManagement()
        {
            if (Session["UsrID"] != null)
            {
                int uID = int.Parse(Session["UsrID"].ToString());
                var KH_ID = dbc.UserWebs.FirstOrDefault(kh => kh.UserID == uID && kh.UserRoles_NVLoaitaikhoan == 3);
                ViewBag.HSDaLuu = dbc.Database.SqlQuery<PHU_HSTD_Luu>("select * from [VLBN].[dbo].[PHU_HSTD_Luu] where UserId =" + uID
        ...
        public ActionResult _CandidateHSPhuHop()
        {
            if (Session["UsrID"] != null)
            {
                int uID = int.Parse(Session["UsrID"].ToString());
        ...
        public ActionResult FacebookCallback(string code)
        {
            ...
            if (!string.IsNullOrEmpty(accessToken))
            {
                ...
                dynamic me = fb.Get("me?fields=id,name,email,gender,link");
        ...
        public ActionResult DN_tuchoiungvien(int KH_ID = 0, int tuyendung_ID = 0, int tuchoi = 2)
        {
            ...
            if (item != null)
            {
                ...
                if (tuchoi == 1)
                {
                    uttuchoi = "TUCHOI";
                    item.DN_TuChoi = uttuchoi;
                    kq = DAO.DN_UngTuyen_Dao.NgungUngTuyen_DoanhNghiep_UngTuyen(dbc, item);
                }
                else
                {
                    item.DN_TuChoi = uttuchoi;
                    kq = DAO.DN_UngTuyen_Dao.Update_DoanhNghiep_UngTuyen(dbc, item);
                }
                if (kq)
                {
                    return Json("OK");
        ...
        public ActionResult DeleteJobSave(int Id)
        {
            var model = dbc.PHU_HSTD_Luu.Find(Id);
            try
            {
                var ten = model.TenHoSo;
                dbc.PHU_HSTD_Luu.Remove(model);
                dbc.SaveChanges();
                ModelState.Clear();//xoa thong tin tren form
                ModelState.AddModelError("", "Xóa Thành Công !!!");
                Session["ThongBao_KH_TimViec"] = "Xóa thành công 1 hồ sơ tuyển dụng do bạn đã lưu.";
                return View("_MainCandidate");
            }
            catch
            {
